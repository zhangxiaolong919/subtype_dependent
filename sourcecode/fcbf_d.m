function [selectedFeatures] = fcbf_d(featureMatrix,classColumn,threshold)
...
for i = 1:numFeatures
    classScore(i) = SU(double(featureMatrix(:,i)),double(classColumn));
end
...
hX = entropy(firstVector);
hY = entropy(secondVector);
iXY = mutualinfo(firstVector,secondVector);
